...
router.post(PATH_PREFIX + "login", (req, res) => {
    ...
    let prom = user_data_controller_1.UserDataController.selectUserPasswordHash(userName);
    prom.then(user_pw_hash => {
        ...
        if (user_1.User.passwordMatchesHash(password, password_hash)) {
            token = user_1.User.createJwtToken(userID, userName);
            console.log(token);
...
router.get(PATH_PREFIX + "users", (req, res) => {
    ...
    let user = new user_1.User();
    user.emailAdress = emailAdress;
    let prom = user_data_controller_1.UserDataController.selectUserByEmail(emailAdress);
    prom.then(user => {
        res.status(200).json(user);
    }).catch(error => {
        console.log(error);
